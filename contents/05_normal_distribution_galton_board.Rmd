...
```{julia}
# set the number of times the loop should be evaluated; this is not part of the for loop syntax
neval = 3; 
for i = 1:neval
  # instructions to be repeated go here 
  println(i^2)
end 
# note the end; this is required to tell Julia where the loop instructions end!
```
...
for i = 3:neval
  # stores the sum of the preceding two elements of `fibonacci` in the ith element of `fibonacci`
...
# specify the normal distribution with mean = 0 and standard deviation = 1, which are the default choices
...
using Plots
gr();
histogram(output, xlabel = "Bin", ylabel = "Count", legend=:false)
```
